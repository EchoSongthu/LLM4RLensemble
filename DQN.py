...
class Agent:
    def __init__(self, mode="training"):
        ...
        self.frames = []
        self._reset()
        self.game_dir=os.path.join('..','model', 'atari', env_name)
        self.time_data=time.strftime('%Y-%m-%d_%H-%M', time.localtime())
        self.model_dir=f'DQN_lr{learning_rate}_seed{manual_seed}_{self.time_data}'
    ...
    def get_experience(self,if_render):
        ...
        if terminate:
            ...
            tqdm.write(f"=={env_name}_DQN== timestep {self.timestep} Score: {episode_reward}")
            self.episode += 1
            self._reset()
    ...
    def update_weights(self):
        ...
        if self.learns % target_update_interval == 0:
            ...
            tqdm.write(f"=={env_name}_DQN== episode {self.episode}: target model weights updated")

    def save(self,step_count):
        torch.save(self.model.state_dict(),os.path.join(self.game_dir,self.model_dir,f'model-{step_count}.pth'))
...
for episode in tqdm(range(episodes)):
    setproctitle.setproctitle(f"RLE@syw_dqn_{env_name}_seed_{manual_seed}_{episode}")
    action_record = []
    action_flag_record = []
    reward_record = []
    terminate = False
    ...
    while not terminate:
        ...
        if terminate:
            ...
            tqdm.write(f"=={env_name}_DQN== episode {episode}, episode reward: {episode_reward}, mean reward: {mean_reward:.3f}")
